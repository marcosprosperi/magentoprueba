...
class Run
{
    ...
    public function run(\Magento\Cron\Model\Schedule $schedule)
    {
        ...
        try {
            ...
            if ($this->_coreHelper->getStoreConfig("massproductimport/settings/enable_reporting")) {
                $emails = explode(',', $this->_coreHelper->getStoreConfig("massproductimport/settings/emails"));
                if (count($emails) > 0) {
                    try {
                        if ($cnt) {
                            $template = "wyomind_massproductimport_cron_report";
                            ...
                            $transport = $this->_transportBuilder
                                    ...
                                    ->setTemplateVars(
                                            [
                                                ...
                                                'subject' => $this->_coreHelper->getStoreConfig('massproductimport/settings/report_title')
                                            ]
                                    )
                                    ->setFrom(
                                            [
                                                'email' => $this->_coreHelper->getStoreConfig('massproductimport/settings/sender_email'),
                                                'name' => $this->_coreHelper->getStoreConfig('massproductimport/settings/sender_name')
                                            ]
                                    )
                                    ->addTo($emails[0]);
