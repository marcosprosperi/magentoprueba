...
var PointOfSale = {
    ...
    displayMyAddress: function (myAddress) {
        ...
        myPlace += "<div>";
        jQuery('#tools').html(myPlace);
        var newAddress = myAddress.split(", ").reverse();
        
        if(newAddress[0]) {
            var result = jQuery.grep(PointOfSale.countries, function(e){ return e.name == newAddress[0]; });
            if (result.length == 1) {
                jQuery("#country_place").val(result[0].code);
                    PointOfSale.createDrop(result[0].code);
                    PointOfSale.updateList();
              }
        }
        if(newAddress[1]) {
            newAddress[1] = newAddress[1].replace(' Province','');
            var result = jQuery.grep(PointOfSale.states, function(e){ return e.name == newAddress[1] });
            if (result.length == 1) {
                jQuery("#state_place").val(result[0].code);
                    PointOfSale.updateList();
              }
        }
        if(newAddress[2]) {
            debugger;
            newAddress[2] = PointOfSale.cleanString(newAddress[2]);
            jQuery("#city").val(newAddress[2]);
            PointOfSale.updateListLike();
        }
    },
    ...
    updateList: function () {
        if (jQuery('#country_place').length) {
            ...
            jQuery('#state_selection').show();
            if (jQuery('#country_place').attr('value') != "*") {
                jQuery('.' + jQuery('#country_place').attr('value')).each(function (c) {
                    jQuery(this).show();
                });
            } else {
                jQuery('#state_selection').hide();  
                jQuery('#city_selection').hide();
                jQuery('#city').val("");
            }
        }
        if((jQuery('#country_place').attr('value') != "*") && (jQuery('#state_place').attr('value') != "*")) {
            jQuery('.place').each(function (c) {
                if(jQuery(this).attr('data-state') != jQuery('#state_place').attr('value')) {
                    jQuery(this).hide();
                }
            });
            jQuery('#city_selection').show();
        } else {
            jQuery('#city_selection').hide();
            jQuery('#city').val("");
        }
    },
    updateListLike: function() {
        var valType = jQuery("#city").attr('value');
        jQuery('.place').each(function(){
            if(jQuery("#country_place").attr('value') == jQuery(this).attr("data-country") && jQuery("#state_place").attr('value') == jQuery(this).attr("data-state")) {
                var val = jQuery(this).attr('data-city').toLowerCase();
                (val.indexOf(valType) != -1) ? jQuery(this).show() : jQuery(this).hide(); 
            }
        });
    },
    ...
    createDrop: function ($countryId) {
        var states = PointOfSale.states;
        if($countryId) {
            var select = "<select id='state_place'><option value='*'>Select a state :</option>";
            states.forEach(function(state, index) {
                if(state.country_id == $countryId) {
                    select += "<option value='" + state.code + "'>"+ state.name +"</option>";
                }
            });
            select += "</select>";
            jQuery('#state_place').remove();
            jQuery('#state_selection').append(select);
        } 
    },
...
require(["jquery", "mage/mage"], function ($) {
    $(function () {
        ...
        $(document).on('change', '#country_place', function () {
            PointOfSale.createDrop($('#country_place').attr('value'));
            PointOfSale.updateList();
        });
        $(document).on('change', '#state_place', function () {
            PointOfSale.updateList();
        });
